...
@section('content')
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                ...
                <a href="{{ route('obras.show', $obra) }}"
                    class="hover:text-slate-900">{{ Str::limit($obra->descricao, 40) }}</a>
                <span>›</span>
        ...
        <div class="flex justify-center">
            <div class="w-full max-w-4xl">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <form method="POST" action="{{ route('obras.execucoes.update', [$obra, $execucao]) }}"
                        ...
                        <div class="px-8 py-6 space-y-8">
                            ...
                            <div>
                                <h3 class="text-sm font-semibold text-slate-500 uppercase mb-4 flex items-center gap-2">
                                    <span
                                        class="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center font-bold">1</span>
                                    Contrato
                                </h3>
                                <div class="space-y-2">
                                    ...
                                    <select name="contrato_id" required @change="selecionarContrato($event.target.value)"
                                        ...
                                        @foreach ($contratos as $c)
                                            <option value="{{ $c->id }}"
                                                {{ old('contrato_id', $execucao->contrato_id) == $c->id ? 'selected' : '' }}>
                            ...
                            <div>
                                <h3 class="text-sm font-semibold text-slate-500 uppercase mb-4 flex items-center gap-2">
                                    <span
                                        class="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center font-bold">2</span>
                                    Dados da Medição
                                </h3>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        ...
                                        <input type="date" name="data_medicao"
                                            value="{{ old('data_medicao', $execucao->data_medicao->format('Y-m-d')) }}"
                                            required
                                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                                    </div>
                                    <div class="space-y-2">
                                ...
                                <div x-show="contratoSelecionado && valorAtual>0" x-cloak
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <p class="font-bold text-indigo-700" x-text="novoPercentual.toFixed(1)+'%'"></p>
                                        </div>
                                    </div>
                                    <div class="mt-4">
                            ...
                            <div x-data="responsaveisManager({{ $responsaveisAtuais->toJson() }}, {{ collect($usuarios)->map(fn($u) => ['id' => $u->id, 'name' => $u->name])->toJson() }})">
                                <h3 class="text-sm font-semibold text-slate-500 uppercase mb-4 flex items-center gap-2">
                                    <span
                                        class="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center font-bold">3</span>
                                ...
                                <div
                                    class="flex flex-wrap gap-3 items-end bg-amber-50 border border-amber-200 rounded-lg p-4">
                            ...
                            @if ($execucao->documentos->isNotEmpty())
                                <div>
                                    <h3
                                        ...
                                        <span
                                            class="w-5 h-5 rounded-full bg-amber-500 text-white text-xs flex items-center justify-center font-bold">4</span>
                                            ...
                                            <div
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <p class="font-medium text-slate-700 truncate">
                                                        {{ $doc->descricao ?? $doc->nome_original }}</p>
                                                    <p class="text-xs text-slate-400">
                                                        {{ $doc->tipo_label }} · {{ $doc->tamanho_legível }} ·
                            ...
                            <div x-data="documentosManager()">
                                        ...
                                        <div
                                            ...
                                            <select :name="'arquivos_tipo[' + idx + ']'"
                                                ...
                                                @foreach ($tiposDocumento as $key => $label)
                                                    <option value="{{ $key }}"
                                                        {{ $key === 'medicao' ? 'selected' : '' }}>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <input type="file" x-ref="fileInput" class="hidden" multiple
                                    accept=".pdf,.jpg,.jpeg,.png,.gif,.webp,.xlsx,.xls,.csv,.docx,.doc" name="arquivos[]"
                                    @change="adicionarArquivos($event)">

                                <div @click="$refs.fileInput.click()" @dragover.prevent
                                    @drop.prevent="adicionarArquivosDrop($event)"
                                    class="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center cursor-pointer hover:border-amber-400 hover:bg-amber-50 transition">
                                    <p class="text-slate-500 text-sm">📎 Clique ou arraste arquivos aqui</p>
                                    <p class="text-xs text-slate-400 mt-1">PDF, imagens, Excel, Word · Máx. 20 MB/arquivo
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="px-8 py-5 border-t bg-slate-50 rounded-b-xl flex justify-between">
                            <a href="{{ route('obras.show', $obra) }}"
                                ...
                                Cancelar</a>
                            <button type="submit"
                                class="px-6 py-2.5 rounded-lg text-sm font-medium text-white bg-amber-600 hover:bg-amber-700">✔
                                Atualizar Medição</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
                ...
                <div class="p-6 space-y-3 text-sm text-slate-700">
                    ...
                    <p>Documentos existentes podem ser baixados ou removidos (somente admin). Novos documentos são
                        adicionados na seção inferior.</p>
                    <p>Responsáveis são substituídos pelos novos selecionados.</p>
                </div>
                <div class="px-6 py-4 border-t flex justify-end">
                    <button @click="modalAjuda=false" class="px-5 py-2 bg-amber-600 text-white rounded-lg">Entendi
                        👍</button>
                </div>
            </div>
        </div>
    ...
    <script>
        function medicaoForm(saldos, valorInicial, contratoIdInicial) {
            return {
                modalAjuda: false,
                contratoSelecionado: saldos[contratoIdInicial] ?? null,
