...
@section('content')
        ...
        <div class="flex justify-center">
            <div class="w-full max-w-4xl">
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <form method="POST" action="{{ route('obras.execucoes.store', $obra) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="px-8 py-6 space-y-8">
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <select name="contrato_id" required @change="selecionarContrato($event.target.value)"
                                        ...
                                        @foreach ($contratos as $c)
                                            <option value="{{ $c->id }}" {{ old('contrato_id') == $c->id ? 'selected' : '' }}>
                                ...
                                <div x-show="contratoSelecionado" x-cloak class="mt-4 grid grid-cols-3 gap-4">
                                    ...
                                    <div class="rounded-lg p-3 text-center border"
                                        :class="(contratoSelecionado?.saldo??1)<=0?'bg-red-50 border-red-200':'bg-blue-50 border-blue-200'">
                                        <p class="text-xs text-slate-500 mb-1">Saldo disponível</p>
                                        <p class="font-semibold text-sm"
                                            :class="(contratoSelecionado?.saldo??1)<=0?'text-red-700':'text-blue-700'"
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="hidden" name="valor_medido" x-ref="valorHidden" value="{{ old('valor_medido') }}">
                                    </div>
                                </div>
                                ...
                                <div x-show="contratoSelecionado && valorAtual > 0" x-cloak
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        <div class="text-center"><p class="text-xs text-slate-500 mb-1">Esta medição</p><p class="font-bold text-slate-800" x-text="'R$ '+fmt(valorAtual)"></p></div>
                                    ...
                                    <div x-show="novoSaldo<0" x-cloak class="mt-3 bg-red-50 border border-red-300 text-red-700 px-3 py-2 rounded text-xs">⚠️ Valor excede o saldo em R$ <span x-text="fmt(Math.abs(novoSaldo))"></span>.</div>
                                </div>
                            ...
                            <div x-data="responsaveisManager()">
                                ...
                                <div class="space-y-2 mb-3">
                                    ...
                                    <p x-show="lista.length===0" class="text-sm text-slate-400 italic py-1">Nenhum responsável adicionado.</p>
                                </div>

                                <div class="flex flex-wrap gap-3 items-end bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    <div class="flex-1 min-w-48 space-y-1">
                                        ...
                                        <select x-model="novoUserId" class="w-full px-3 py-2 rounded-lg border border-slate-300 text-sm">
                                            ...
                                            @foreach ($usuarios as $u)
                                                <option value="{{ $u->id }}" data-nome="{{ $u->name }}">{{ $u->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="space-y-1">
                                    ...
                                    <button type="button" @click="adicionar()" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">➕ Adicionar</button>
                                </div>
                            </div>

                            {{-- 4. DOCUMENTOS --}}
                            {{-- 4. DOCUMENTOS — input simples e direto, sem DataTransfer --}}
                            <div x-data="{ docs: [] }">
                                <h3 class="text-sm font-semibold text-slate-500 uppercase mb-4 flex items-center gap-2">
                                    <span class="w-5 h-5 rounded-full bg-blue-600 text-white text-xs flex items-center justify-center font-bold">4</span>
                                    Documentos Anexos
                                    <span class="text-xs font-normal text-slate-400 normal-case">(boletim, fotos, planilhas — opcional)</span>
                                </h3>

                                {{-- LISTA DOS ARQUIVOS SELECIONADOS (apenas visual) --}}
                                <div class="space-y-1 mb-3">
                                    <template x-for="(doc, idx) in docs" :key="idx">
                                        <div class="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm">
                                            <span class="text-lg shrink-0" x-text="iconeArq(doc.nome)"></span>
                                            <div class="flex-1 min-w-0">
                                                <p class="font-medium text-slate-700 truncate" x-text="doc.nome"></p>
                                                <p class="text-xs text-slate-400" x-text="tamanhoArq(doc.tamanho)"></p>
                                            </div>
                                        </div>
                                    </template>
                                </div>

                                {{-- INPUT REAL — visível e simples, o browser gerencia os arquivos --}}
                                <div class="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:border-blue-400 hover:bg-blue-50 transition">
                                    ...
                                    <p class="text-xs text-slate-400 mt-2">PDF, imagens, Excel, Word · Máx. 20 MB/arquivo · Até 10 arquivos</p>
                                </div>

                                {{-- TIPO E DESCRIÇÃO — campos simples quando há arquivos --}}
                        ...
                        <div class="px-8 py-5 border-t bg-slate-50 rounded-b-xl flex justify-between">
                            ...
                            <button type="submit" class="px-6 py-2.5 rounded-lg text-sm font-medium text-white bg-green-600 hover:bg-green-700">✔ Registrar Medição</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden">
                ...
                <div class="p-6 space-y-4 text-sm text-slate-700">
                    ...
                    <p><strong>Documentos:</strong> Boletim assinado, fotos da obra, planilha de serviços. PDF, imagens, Excel, Word.</p>
                    <div class="bg-slate-50 p-4 rounded text-xs">💡 Saldo e % são calculados automaticamente.</div>
                </div>
                <div class="px-6 py-4 border-t flex justify-end">
                    <button @click="modalAjuda=false" class="px-5 py-2 bg-green-600 text-white rounded-lg">Entendi 👍</button>
                </div>
            </div>
        </div>
    ...
    <script>
        function medicaoForm(saldos) {
            return {
                modalAjuda: false, contratoSelecionado: null, valorAtual: 0,
        ...
        function responsaveisManager() {
            return {
                lista:[], novoUserId:'', novoPapel:'engenheiro',
                adicionar() {
                    ...
                    this.lista.push({user_id:this.novoUserId,papel:this.novoPapel,nome:sel?.dataset.nome??''});
                    this.novoUserId='';
                },
        ...
        // Helpers globais usados inline na seção de documentos
        function iconeArq(n) {
            const e = n.split('.').pop().toLowerCase();
            return {pdf:'📄',jpg:'🖼️',jpeg:'🖼️',png:'🖼️',gif:'🖼️',webp:'🖼️',xlsx:'📊',xls:'📊',csv:'📊',docx:'📝',doc:'📝'}[e]??'📎';
        }
        function tamanhoArq(b) {
            return b>=1048576?(b/1048576).toFixed(1)+' MB':b>=1024?Math.round(b/1024)+' KB':b+' bytes';
        }
    </script>
