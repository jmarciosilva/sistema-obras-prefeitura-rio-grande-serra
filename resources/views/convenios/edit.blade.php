...
@section('subtitle', 'Atualização de dados do convênio')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="px-8 py-6 border-b border-slate-200">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-slate-900">Editar Convênio</h2>
                                <p class="text-sm text-slate-600 mt-1">Atualize as informações do convênio</p>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($vencido)
                        <div class="mx-8 mt-6 bg-red-50 border border-red-300 text-red-700 px-4 py-3 rounded-lg text-sm">
                            ⚠️ Este convênio está com a <strong>vigência vencida</strong>.
                        </div>
                    @elseif ($breve)
                        <div class="mx-8 mt-6 bg-yellow-50 border border-yellow-300 text-yellow-700 px-4 py-3 rounded-lg text-sm">
                            ⚠️ Este convênio vence em menos de <strong>30 dias</strong>.
                        </div>
                    @endif
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <select name="categoria_convenio_id" required
                                            ...
                                            @foreach ($categorias as $cat)
                                                <option value="{{ $cat->id }}"
                                                    {{ old('categoria_convenio_id', $convenio->categoria_convenio_id) == $cat->id ? 'selected' : '' }}>
                                                    {{ $cat->nome }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <select name="orgao_financiador_id" required
                                            ...
                                            @foreach ($orgaos as $o)
                                                <option value="{{ $o->id }}"
                                                    {{ old('orgao_financiador_id', $convenio->orgao_financiador_id) == $o->id ? 'selected' : '' }}>
                                                    {{ $o->nome }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <textarea name="descricao" rows="3" required
                                        class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 resize-none">{{ old('descricao', $convenio->descricao) }}</textarea>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="text" name="numero_convenio_ano"
                                            value="{{ old('numero_convenio_ano', $convenio->numero_convenio_ano) }}"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="text" name="processo_pref_concedente"
                                            value="{{ old('processo_pref_concedente', $convenio->processo_pref_concedente) }}"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="text" name="cadastro_prescon_num"
                                            value="{{ old('cadastro_prescon_num', $convenio->cadastro_prescon_num) }}"
                                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                                    </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="date" name="assinatura"
                                            value="{{ old('assinatura', $convenio->assinatura?->format('Y-m-d')) }}"
                                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                                    </div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input type="date" name="vigencia"
                                            value="{{ old('vigencia', $convenio->vigencia?->format('Y-m-d')) }}"
                                            class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                                    </div>
                                ...
                                <div class="max-w-sm space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text"
                                            ...
                                            value="{{ old('valor_repasse_contrapartida', $convenio->valor_repasse_contrapartida ? number_format((float) $convenio->valor_repasse_contrapartida, 2, ',', '.') : '') }}"
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4">
                    <h3 class="text-lg font-semibold">📘 Ajuda — Edição de Convênio</h3>
                    <p class="text-sm opacity-90">Como atualizar o convênio</p>
                </div>
                <div class="p-6 space-y-4 text-sm text-slate-700">
                    <p>Atualize os dados conforme necessário. <strong>Categoria</strong> e <strong>Órgão Financiador</strong> são obrigatórios.</p>
                    <div class="bg-slate-50 p-4 rounded text-xs">
                        💡 Alterações no valor não afetam os registros de execução já lançados.
                    </div>
                </div>
