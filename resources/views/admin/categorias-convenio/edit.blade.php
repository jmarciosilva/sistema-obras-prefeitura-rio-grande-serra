...
@section('subtitle', 'Atualização de categoria de convênio')
    ...
    <div x-data="{ modalAjuda: false }">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                ...
                <span class="text-slate-900 font-medium">Editar: {{ $categoria->nome }}</span>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="px-8 py-6 border-b border-slate-200">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-slate-900">Editar Categoria</h2>
                                <p class="text-sm text-slate-600 mt-1">Atualize os dados da categoria
                                    <strong>{{ $categoria->nome }}</strong></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.categorias-convenio.update', $categoria) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="px-8 py-6 space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="descricao" rows="3"
                                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 resize-none">{{ old('descricao', $categoria->descricao) }}</textarea>
                            </div>
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4">
                    <h3 class="text-lg font-semibold">📘 Ajuda — Edição de Categoria</h3>
                </div>
                <div class="p-6 space-y-3 text-sm text-slate-700">
                    <p>Atualize nome e descrição. A alteração do nome reflete imediatamente em todos os convênios vinculados
                        a esta categoria.</p>
                </div>
