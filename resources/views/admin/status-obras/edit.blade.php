...
@section('subtitle', 'Atualização de status de obra')

@section('content')

    <div x-data="statusForm('{{ old('cor', $status->cor) }}')">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                ...
                <span class="text-slate-900 font-medium">Editar: {{ $status->nome }}</span>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="px-8 py-6 border-b border-slate-200">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-slate-900">Editar Status</h2>
                                <p class="text-sm text-slate-600 mt-1">
                                    Atualize as configurações do status
                                    <span
                                        class="inline-flex items-center ml-1 px-2 py-0.5 rounded-full text-xs text-white font-medium"
                    ...
                    <form method="POST" action="{{ route('admin.status-obras.update', $status) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="px-8 py-6 space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="nome" value="{{ old('nome', $status->nome) }}" required
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4">
                    <h3 class="text-lg font-semibold">📘 Ajuda — Edição de Status</h3>
                </div>
                <div class="p-6 space-y-3 text-sm text-slate-700">
                    <p>Atualize as configurações do status. A alteração de cor reflete imediatamente em todas as obras que
                        usam este status.</p>
                    <div class="bg-slate-50 p-3 rounded text-xs">
                        💡 Alterar o nome do status não afeta os registros existentes — obras continuam vinculadas ao mesmo
                        registro.
                    </div>
                </div>
    ...
    <script>
        function statusForm(corInicial) {
            return {
                modalAjuda: false,
                cor: corInicial || '#6c757d',
