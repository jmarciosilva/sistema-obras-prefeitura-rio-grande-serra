    ...
    <div x-data="{ modalAjuda: false }">
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-2 text-sm text-slate-600">
                ...
                <span>›</span><span class="text-slate-900 font-medium">Editar: {{ $demanda->numero_demanda }}</span>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    <div class="px-8 py-6 border-b">
                        <div class="flex items-start gap-4">
                            <div class="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center text-xl">✏️</div>
                            <div>
                                <h2 class="text-xl font-semibold text-slate-900">Editar Demanda</h2>
                                <p class="text-sm font-mono text-slate-500 mt-1">{{ $demanda->numero_demanda }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.demandas-propostas.update', $demanda) }}">
                        @csrf @method('PUT')
                        <div class="px-8 py-6 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="numero_demanda"
                                        value="{{ old('numero_demanda', $demanda->numero_demanda) }}" required
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input type="date" name="data_solicitacao"
                                        value="{{ old('data_solicitacao', $demanda->data_solicitacao?->format('Y-m-d')) }}"
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="descricao" rows="2" required
                                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 resize-none">{{ old('descricao', $demanda->descricao) }}</textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div class="space-y-2">
                                    ...
                                    <select name="origem" required
                                        ...
                                        @foreach ($origens as $val => $label)
                                            <option value="{{ $val }}"
                                                {{ old('origem', $demanda->origem) === $val ? 'selected' : '' }}>
                                                {{ $label }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <select name="situacao" required
                                        ...
                                        @foreach ($situacoes as $val => $label)
                                            <option value="{{ $val }}"
                                                {{ old('situacao', $demanda->situacao) === $val ? 'selected' : '' }}>
                                                {{ $label }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="solicitante"
                                        value="{{ old('solicitante', $demanda->solicitante) }}"
                                        class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500">
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="observacoes" rows="3"
                                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 resize-none">{{ old('observacoes', $demanda->observacoes) }}</textarea>
                            </div>
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
                ...
                <div class="p-6 text-sm text-slate-700">
                    <p>Atualize a situação da demanda conforme o andamento. Quando aprovada e virar uma obra, vincule-a no
                        cadastro da obra.</p>
                </div>
