...
@section('title', 'Editar Responsável Técnico')
@section('subtitle', 'Atualização de dados do responsável técnico')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200">
                    ...
                    <div class="px-8 py-6 border-b border-slate-200">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h2 class="text-xl font-semibold text-slate-900">Editar Responsável Técnico</h2>
                                <p class="text-sm text-slate-600 mt-1">{{ $responsavel->nome }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('admin.responsaveis-tecnicos.update', $responsavel) }}">
                        @csrf
                        @method('PUT')
        ...
        <div x-show="modalAjuda" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl overflow-hidden">
                <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4">
                    <h3 class="text-lg font-semibold">📘 Ajuda — Editar Responsável Técnico</h3>
                </div>
